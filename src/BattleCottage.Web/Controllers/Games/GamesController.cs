...
public class GamesController : APIControllerBase
{
    ...
    public async Task<IActionResult> AllGames([FromQuery] string? contains, string? page, string? pageSize)
    {
        ...
        if (games == null || games.Count == 0) return NotFound(new MessageResponse("No games found."));

        var pageNumber = string.IsNullOrEmpty(page) ? PageSettings.FirstPageNumber : int.Parse(page);
        var size = string.IsNullOrEmpty(pageSize) ? PageSettings.MaxPageSize : int.Parse(pageSize);
